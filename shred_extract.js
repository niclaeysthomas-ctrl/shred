...
function renderJour(){
  ...
  const blocks=[
    {id:'juke', ic:'🎧', t:'Jouer sur un morceau', d:'Le Juke-box — la grille tourne, tu improvises. '+jbCount()+'/'+JUKE.length+' validés.', open:()=>openJukebox()},
    (dueBox
...
function renderProfil(){
  ...
  tc.appendChild(el('div','statgrid',
    `<div class="stat"><b>${Math.round(tot/60)}</b><small>minutes au total</small></div>
     ...
     <div class="stat"><b>${takes().length}</b><small>prises gardées</small></div>`));
  ...
  const mc=el('div','card'); mc.innerHTML='<b>Ta maîtrise</b>'+bar('Triades (leurs notes)',triK,triT)+bar('Formes d’accords',chK,chT)+bar('Cordes du manche',cleanStrings,6)+bar('Boîtes de gamme conquises',bxMastered(),5)+bar('Morceaux joués (Juke-box)',jbCount(),JUKE.length);
  m.appendChild(mc);
